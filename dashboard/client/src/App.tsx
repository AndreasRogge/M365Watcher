...
function AppContent() {
  const { mode, isAuthenticated, loading, msalReady, getAccessToken } = useAuth();
  ...
  // Wait for MSAL to finish processing any pending redirect before showing
  // LoginPage. This prevents a brief flash of the login screen while
  // handleRedirectPromise() is exchanging the auth code for tokens.
  if (mode === "user" && !isAuthenticated && !msalReady) {
    return null;
  }
